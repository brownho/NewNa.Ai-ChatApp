...
class MeetingMentorFullscreen {
    ...
    hide() {
        // Show chat container - check if it exists first
        const chatContainer = document.querySelector('.container');
        if (chatContainer) {
            chatContainer.style.display = 'flex';
        } else {
            console.error('Chat container not found');
            // Try to go back to index.html if container not found
            window.location.href = 'index.html';
            return;
        }
        
        // Hide fullscreen meeting interface
        const meetingFS = document.getElementById('meeting-mentor-fullscreen');
        if (meetingFS) {
            meetingFS.style.display = 'none';
        }
