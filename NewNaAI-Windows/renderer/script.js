...
window.addEventListener('load', async () => {
    ...
    // Skip service worker for Electron app
    ...
    if (!userStr) {
        ...
        window.location.href = 'login.html';  // Relative path for Electron
        return;
    }
    ...
    // Skip session validation for Electron app - rely on stored user data
    if (!isGuest && window.electronAPI) {
        console.log('Electron app detected, skipping session validation');
        // Show dashboard link based on cached user data
        ...
        // Only validate session for web version
        try {
...
async function loadModels() {
    try {
        ...
        if (Array.isArray(models) && models.length > 0) {
            ...
            const fallbackModels = ['mixtral', 'llama2', 'codellama', 'mistral'];
            ...
            console.warn('Using fallback models due to API response:', data);
        }
        ...
        setStatus('error', 'Failed to load models - using defaults');
...
async function updateGPUStats() {
    ...
    try {
        ...
        if (!response.ok) {
            // API not available, just hide the stats
        ...
        // Silently fail - GPU stats are optional
        gpuTempSpan.textContent = '--°C';
        gpuMemorySpan.textContent = '--GB';
    }
}
