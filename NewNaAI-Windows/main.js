  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const request = net.request({
        ...
        useSessionCookies: true // Enable session cookies
      });
      ...
      // Handle credentials - don't set as header, it's handled by useSessionCookies
      if (options.credentials === 'include') {
        // Cookies are automatically included with useSessionCookies: true
      }
...
function createWindow() {
  ...
  const isTestAuth = process.argv.includes('--test-auth');
  let startUrl;
  
  if (isTestAuth) {
    // Load auth test page
    startUrl = `file://${path.join(__dirname, 'renderer/test-auth.html')}`;
  } else if (isDev) {
...
app.whenReady().then(() => {
  ...
  ses.setCertificateVerifyProc((request, callback) => {
    // Always allow certificates (for development)
    callback(0);
  });
  
  // Also configure the named partition
  const namedSession = session.fromPartition('persist:newnaai');
  namedSession.setCertificateVerifyProc((request, callback) => {
    callback(0);
  });
